...
busSuccess2 <- function( nreps ) {
  ...
  probabilities <- c( 0.5, 0.25, 0.15, 0.1 )
  # Simulate arrival times and find the difference
  successes <- Reduce( "-", list( sample( delays, nreps, TRUE, probabilities ),
                              sample( delays, nreps, TRUE, probabilities ) ) )
  # Create new vector with only successful transfers
  result <- successes[ successes <= 1.25 ]
  return( length( result[ result < 0.0 ] ) / length( result ) )
}
